...
// Reads the Contact page chrome (hero, aside, office hours, labels, map) from the
// contact_page singleton. Email / phone / address / socials come from
// useSiteSettings() — this hook does NOT duplicate them.
export function useContactContent() {
  const { databases, config } = useMemo(() => createDatabasesClient(), []);
  const [content, setContent] = useState(CONTACT_DEFAULTS);

  useEffect(() => {
    if (!databases || !config.databaseId || !config.collections.contactPage) return;
    let mounted = true;
    ...
    (async () => {
      try {
        const res = await databases.listDocuments(
          config.databaseId,
          config.collections.contactPage,
          [Query.orderDesc("$createdAt"), Query.limit(1)],
        );
        const doc = res.documents?.[0];
        if (!doc || !mounted) return;
        setContent(mergeDefined(CONTACT_DEFAULTS, doc));
      } catch {
        // keep defaults
      }
    })();
